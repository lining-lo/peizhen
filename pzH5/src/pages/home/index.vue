<template>
    ...
    <van-swipe class="my-swiper">
        <van-swipe-item v-for="item in homeData.slides" :key="item.id">
            <van-image radius="10" :src="item.pic_image_url" />
        </van-swipe-item>
    </van-swipe>
    <!-- 快捷卡片 -->
    <van-row justify="space-around">
        <van-col span="11" v-for="(item, index) in homeData.nav2s" :key="item.id">
            <van-image @click="goOrderTwo(index)" radius="10" :src="item.pic_image_url" />
        </van-col>
    </van-row>
    <!-- 医院列表 -->
